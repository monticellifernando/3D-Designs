...
//        translate([35,-20,0])
        ...
        scale(Tamano/16)
        import("question.stl", convexity=6);
}
...
module SacarInterior(Tamano,AjusteInterior){
    AgujeroInterior=Tamano*80/100-AjusteInterior;
        union(){
...
module Rejilla(Tamano=100){
    ...
    difference(){
        translate([0,0,0.8*Tamano]) cube([0.78*Tamano-0.5,0.78*Tamano-0.5,2],center=true);
        for (i=[0:NroDeRayos]){
...
module Maceta(Tamano=100){
    difference(){
        ...
        translate([0,0,Tamano*.43])
            cube([Tamano*.8-5,Tamano*.8-5,Tamano], center=true);
        Base(Tamano,1);
        translate([0,0,1.25*Tamano])
            cube([Tamano*.8,Tamano*.8,Tamano*2], center=true);
    }
